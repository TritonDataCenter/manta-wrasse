...
function streamJob(opts, cb) {
    ...
    var put_opts = {
        headers: {
            ...
            'role-tag': opts.job.conditions && opts.job.conditions.activeRoles
        },
...
function mkdir(opts, cb) {
    ...
    var key = '/' + opts.account + '/jobs/' + opts.job.jobId;
    var log = opts.log;
    var _opts = {
        headers: {
            'access-control-allow-origin': '*',
...
function upload(opts) {
    ...
    function _upload(job, cb) {
        ...
        // have a partial upload, but meh. They did mrm -r blindly on us.
        vasync.pipeline({
            funcs: [
                createScratchDir,
                startHeartbeat,
