...
var JOB_ROOT_FMT = '/%s/stor/jobs/%s';
var JOB_INPUT_FMT = JOB_ROOT_FMT + '/in';
var JOB_OUTPUT_FMT = JOB_ROOT_FMT + '/out';
var JOB_ERROR_FMT = JOB_ROOT_FMT + '/err';
var JOB_FAIL_FMT = JOB_ROOT_FMT + '/fail';
...
function uploadJobManifest(opts, cb) {
    ...
    var client = opts.marlin;
    var dir = sprintf(JOB_ROOT_FMT, opts.user.login, opts.job.jobId);
    var job = opts.job;
    var log = opts.log;
    var m = opts.manta;
    var stream = new MemoryStream();
    ...
    m.mkdirp(dir, function (mkdir_err) {
        if (mkdir_err) {
            log.error(mkdir_err, 'uploadJob: mkdirp error');
            cb(mkdir_err);
            return;
        }

        var data = JSON.stringify(libmanta.translateJob(job));
        var key = sprintf(JOB_MANIFEST_FMT, opts.user.login, job.jobId);
        var md5 = crypto.createHash('md5');
        var put_opts = {
            headers: {
                'access-control-allow-origin': '*'
            },
            md5: md5.update(data).digest('base64'),
            size: Buffer.byteLength(data),
            type: 'application/json'
        };
        m.put(key, stream, put_opts, function (err) {
            if (err) {
        ...
        vasync.pipeline({
            funcs: [
                function user(opts, cb) {
                    mahi.loadUser(opts, function (err, user) {
                        if (err) {
                            cb(err);
                        } else {
                            opts.user = user;
